...
- 收藏页：顶部使用下划线样式的全部、动画、书籍、音乐、游戏、三次元切换条筛选条目类别，收藏状态作为子切换条；列表下滑触底自动加载后续数据。
- 收藏管理：可更新收藏状态；书籍可更新话数/卷数进度，动画可进入单集进度页批量标记想看、看过、抛弃或未收藏。
- 搜索页：顶部使用下划线样式的全部、动画、书籍、音乐、游戏、三次元、人物切换条切换搜索分类；结果下滑触底自动加载后续数据，条目和人物结果可进入详情页，并支持加入对应收藏。
